...
func TestPluginDescribe(t *testing.T) {
	d := plugin{}.Describe()
	if d.ID != "webhook" || d.Label == "" {
		t.Errorf("Describe() = ID %q, Label %q; want webhook, non-empty", d.ID, d.Label)
	}
...
func TestPluginBuild(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			inst := app.Instance{App: "webhook", Name: "webhook:deploy", Secrets: tt.secrets}
			adapter, err := plugin{}.Build(inst, app.Env{})
			if err != nil {
				t.Fatalf("Build: %v", err)
			}
			if adapter.Name() != "webhook:deploy" {
				t.Errorf("Name() = %q, want webhook:deploy", adapter.Name())
			}
			if adapter.HTTPHandler() == nil {
				t.Error("HTTPHandler() = nil, want handler")
			}
		})
	}
}
