...
func TestPluginDescribe(t *testing.T) {
	d := plugin{}.Describe()
	if d.ID != "msteams" || d.Label == "" {
		t.Errorf("Describe() = ID %q, Label %q; want msteams, non-empty", d.ID, d.Label)
	}
...
func TestPluginBuild(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			inst := app.Instance{App: "msteams", Name: "msteams:ci", Secrets: tt.secrets}
			adapter, err := plugin{}.Build(inst, app.Env{})
			if err != nil {
				t.Fatalf("Build: %v", err)
			}
			if adapter.Name() != "msteams:ci" {
				t.Errorf("Name() = %q, want msteams:ci", adapter.Name())
			}
		})
	}
}
