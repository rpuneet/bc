...
var commonEventTypes = []string{"issue", "comment", "project", "cycle"}

// Adapter implements gateway.NotificationAdapter for Linear webhooks.
type Adapter struct {
...
func New(secret string) *Adapter             { return &Adapter{name: "linear", secret: secret} }
...
func (a *Adapter) HTTPHandler() http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		log.Info("linear: received webhook", "event", eventType, "sender", sender, "adapter", a.name)
		if a.handler != nil {
			a.handler(gateway.Notification{Channel: eventType, Platform: "linear", Sender: sender, Timestamp: now, Raw: body})
		}
...
func (a *Adapter) Channels() []gateway.ChannelInfo {
	...
	for i, e := range commonEventTypes {
		ch[i] = gateway.ChannelInfo{ID: e, Name: e, Platform: "linear"}
	}
	return ch
}
...
func extractSender(body []byte) string {
	...
	if err := json.Unmarshal(body, &p); err == nil {
		if p.Data.User.Name != "" {
			return p.Data.User.Name
		}
		if p.CreatorUser.Name != "" {
			return p.CreatorUser.Name
		}
	}
	return "linear"
}
