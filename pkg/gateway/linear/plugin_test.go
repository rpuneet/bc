...
func TestPluginDescribe(t *testing.T) {
	d := plugin{}.Describe()
	if d.ID != "linear" || d.Label == "" {
		t.Errorf("Describe() = ID %q, Label %q; want linear, non-empty", d.ID, d.Label)
	}
...
func TestPluginBuild(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			inst := app.Instance{App: "linear", Name: "linear:ci", Secrets: tt.secrets}
			adapter, err := plugin{}.Build(inst, app.Env{})
			if err != nil {
				t.Fatalf("Build: %v", err)
			}
			if adapter.Name() != "linear:ci" {
				t.Errorf("Name() = %q, want linear:ci", adapter.Name())
			}
		})
	}
}
