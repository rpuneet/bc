...
func TestPluginDescribe(t *testing.T) {
	d := plugin{}.Describe()
	if d.ID != "sentry" || d.Label == "" {
		t.Errorf("Describe() = ID %q, Label %q; want sentry, non-empty", d.ID, d.Label)
	}
...
func TestPluginBuild(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			inst := app.Instance{App: "sentry", Name: "sentry:ci", Secrets: tt.secrets}
			adapter, err := plugin{}.Build(inst, app.Env{})
			if err != nil {
				t.Fatalf("Build: %v", err)
			}
			if adapter.Name() != "sentry:ci" {
				t.Errorf("Name() = %q, want sentry:ci", adapter.Name())
			}
		})
	}
}
