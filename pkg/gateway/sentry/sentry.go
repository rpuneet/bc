...
var commonEventTypes = []string{"error", "issue", "metric_alert"}

// Adapter implements gateway.NotificationAdapter for Sentry webhooks.
type Adapter struct {
...
func New(secret string) *Adapter              { return &Adapter{name: "sentry", secret: secret} }
...
func (a *Adapter) HTTPHandler() http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		log.Info("sentry: received webhook", "event", eventType, "sender", sender, "adapter", a.name)
		if a.handler != nil {
			a.handler(gateway.Notification{Channel: eventType, Platform: "sentry", Sender: sender, Timestamp: now, Raw: body})
		}
...
func (a *Adapter) Channels() []gateway.ChannelInfo {
	...
	for i, e := range commonEventTypes {
		ch[i] = gateway.ChannelInfo{ID: e, Name: e, Platform: "sentry"}
	}
	return ch
}
...
func extractSender(body []byte) string {
	...
	if err := json.Unmarshal(body, &p); err == nil && p.Actor.Name != "" {
		return p.Actor.Name
	}
	return "sentry"
}
