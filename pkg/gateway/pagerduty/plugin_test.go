...
func TestPluginDescribe(t *testing.T) {
	d := plugin{}.Describe()
	if d.ID != "pagerduty" || d.Label == "" {
		t.Errorf("Describe() = ID %q, Label %q; want pagerduty, non-empty", d.ID, d.Label)
	}
...
func TestPluginBuild(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			inst := app.Instance{App: "pagerduty", Name: "pagerduty:ci", Secrets: tt.secrets}
			adapter, err := plugin{}.Build(inst, app.Env{})
			if err != nil {
				t.Fatalf("Build: %v", err)
			}
			if adapter.Name() != "pagerduty:ci" {
				t.Errorf("Name() = %q, want pagerduty:ci", adapter.Name())
			}
		})
	}
}
