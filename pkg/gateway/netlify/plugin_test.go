...
func TestPluginDescribe(t *testing.T) {
	d := plugin{}.Describe()
	if d.ID != "netlify" || d.Label == "" {
		t.Errorf("Describe() = ID %q, Label %q; want netlify, non-empty", d.ID, d.Label)
	}
...
func TestPluginBuild(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			inst := app.Instance{App: "netlify", Name: "netlify:ci", Secrets: tt.secrets}
			adapter, err := plugin{}.Build(inst, app.Env{})
			if err != nil {
				t.Fatalf("Build: %v", err)
			}
			if adapter.Name() != "netlify:ci" {
				t.Errorf("Name() = %q, want netlify:ci", adapter.Name())
			}
		})
	}
}
