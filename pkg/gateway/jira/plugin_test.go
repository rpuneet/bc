...
func TestPluginDescribe(t *testing.T) {
	d := plugin{}.Describe()
	if d.ID != "jira" || d.Label == "" {
		t.Errorf("Describe() = ID %q, Label %q; want jira, non-empty", d.ID, d.Label)
	}
...
func TestPluginBuild(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			inst := app.Instance{App: "jira", Name: "jira:ci", Secrets: tt.secrets}
			adapter, err := plugin{}.Build(inst, app.Env{})
			if err != nil {
				t.Fatalf("Build: %v", err)
			}
			if adapter.Name() != "jira:ci" {
				t.Errorf("Name() = %q, want jira:ci", adapter.Name())
			}
		})
	}
}
