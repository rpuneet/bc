...
// TestStore_MigrateMCPServersNoDeadlock populates the legacy mcp_servers table and
// ensures Store.Open() completes without hanging. Regression test for a deadlock
// where migrateMCPServers held an open Rows cursor on mcp_servers while issuing
// QueryRowContext on the same SQLite pool, blocking concurrent writers on boot.
func TestStore_MigrateMCPServersNoDeadlock(t *testing.T) {
	setupSharedDB(t)

	// Seed legacy mcp_servers table with a handful of rows (mirrors real workspace).
	shared := db.Shared()
	if _, err := shared.ExecContext(context.Background(), `
		...
		t.Fatalf("create mcp_servers: %v", err)
	}
	legacy := []struct {
		name, transport, command, url string
	}{
		{"github", "stdio", "github-mcp-server", ""},
		{"playwright", "sse", "", "http://host.docker.internal:3000/sse"},
		{"playwright-visible", "sse", "", "http://host.docker.internal:3001/sse"},
		{"bc", "sse", "", "http://host.docker.internal:9374/_mcp/sse"},
	}
	for _, r := range legacy {
		if _, err := shared.ExecContext(context.Background(),
			`INSERT INTO mcp_servers (name, transport, command, url, args, env, enabled)
			 VALUES (?, ?, ?, ?, '[]', '{}', 1)`,
			r.name, r.transport, r.command, r.url); err != nil {
			t.Fatalf("seed mcp_servers %s: %v", r.name, err)
		}
	}

	// Run Open() in a goroutine with a hard timeout — a deadlock would block forever.
	...
	case <-time.After(10 * time.Second):
		t.Fatal("Open() deadlocked — migrateMCPServers held cursor while issuing dependent queries")
	}
	defer s.Close() //nolint:errcheck // test cleanup

	// Verify the 4 legacy MCPs got copied into tools (bc was already a builtin, so it may
	// have been seeded as builtin; we only require that the migration path completed).
	for _, r := range legacy {
		tl, err := s.Get(context.Background(), r.name)
		if err != nil {
			t.Fatalf("Get %s: %v", r.name, err)
		}
		if tl == nil {
			t.Errorf("expected %s to be present after migration", r.name)
		}
	}
}
