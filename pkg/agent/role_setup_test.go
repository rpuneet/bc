...
func TestBcSelfURL(t *testing.T) {
	tests := []struct {
		...
		{"docker rewrites localhost", "http://localhost:9000", "docker", "a1", "http://host.docker.internal:9000/_mcp/a1"},
		{"empty hostname normalized", "http://:8080", "tmux", "a1", "http://127.0.0.1:8080/_mcp/a1"},
		{"no env falls back to default port tmux", "", "tmux", "a1", "http://127.0.0.1:9374/_mcp/a1"},
		{"no env falls back to default port docker", "", "docker", "a1", "http://host.docker.internal:9374/_mcp/a1"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
