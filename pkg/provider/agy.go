...
func (p *AgyProvider) ParseTranscriptLine(line []byte) ([]TranscriptActivity, error) {
	...
	case "RUN_COMMAND":
		// Tool execution results - map PostToolUse based on content
		// agy doesn't include tool name in RUN_COMMAND, so we can't
		// accurately map PostToolUse without state tracking.
		// For now, skip PostToolUse - it will be handled by PreToolUse
		return nil, nil
...
func extractAgyPrompt(content string) string {
	...
	// Also strip <ADDITIONAL_METADATA> if present
