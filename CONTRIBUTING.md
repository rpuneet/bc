...
**Changelog:** GitHub release notes are auto-generated by GoReleaser from commits
since the previous `vMAJOR.MINOR.PATCH` tag. The Release workflow sets
`GORELEASER_PREVIOUS_TAG` via `scripts/previous-semver-tag.sh` so QA/evidence
tags (e.g. `qa-3624-*-evidence`) never become the previous boundary. Do **not**
create non-`v*` tags on release history if you can avoid it; if you must, the
script still ignores them. Hand-edit `CHANGELOG.md` only for security /
breaking / upgrade-action notes (see [Changelog entries](#changelog-entries)).
