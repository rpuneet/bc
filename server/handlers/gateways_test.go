...
func TestGatewayChannelSendRequiresContent(t *testing.T) {
	tests := []struct {
		name       string
		body       string
		wantStatus int
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			req := httptest.NewRequest(http.MethodPost, "/api/gateways/slack/channels/eng/send", strings.NewReader(tt.body))
			rr := httptest.NewRecorder()
...
func TestGatewayChannelSendMethodNotAllowed(t *testing.T) {
	gw := gateway.NewManager()
	h := &GatewayHandler{gw: gw}
	...
	if rr.Code != http.StatusMethodNotAllowed {
		t.Errorf("GET send: got status %d, want 405", rr.Code)
	}
}
